...
const SocialLogins: React.FC<SocialLoginsProps> = ({ isLoading }) => {
      ...
      if (data.session?.user) {
        const queryPromise = supabase
          ...
          .upsert({
            ...
            username: data.session.user.email?.split("@")[0] || "user_" + data.session.user.id,
            is_admin: false,
          });
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error("user_profile upsert timed out after 30s")), 30000)
        );
        const { error: profileError } = await Promise.race([queryPromise, timeoutPromise]);
